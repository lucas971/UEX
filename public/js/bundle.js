...
const InitializeShaders = (d) => {
    ...
    const depthTexture = new d.THREE.DepthTexture();
    const renderTarget = new d.THREE.WebGLRenderTarget(
        window.innerWidth,
        window.innerHeight,
        {
    ...
    // Initial render pass.
    d.composer = new EffectComposer(d.renderer, renderTarget);
    const pass = new RenderPass(d.scene, d.camera);
    d.composer.addPass(pass);

    // Outline pass.
    const customOutline = new CustomOutlinePass(
        new d.THREE.Vector2(window.innerWidth, window.innerHeight),
        d.scene,
        d.camera
    );
    d.composer.addPass(customOutline);
